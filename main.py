...
def get_rdata(rquery):
  ...
  rdf = pd.DataFrame(
    data, columns=['date', 'user', 'rawContent', 'inReplyToTweetId'])
  return rdf
...
for i in range(len(users)):
  query = "(from:" + users[
    i] + ") until:" + until + " since:" + since + " -filter:replies"
...
for i in range(len(users)):
  ...
  # str_sentiment_tw[i] = str_sentiment_tw[i].replace('{', '').replace('}', '')
  str_sentiment_rp[i] = sentiment_rp[i].to_string(index=False, header=False)
  # str_sentiment_rp[i] = str_sentiment_rp[i].replace('{', '').replace('}', '')
  
  active_user[i] = Counter(
    df_rp[i]['user'].apply(lambda x: x['username'])).most_common(5)
  str_Tweets[i] = '\n'.join(list(Tweets[i]))
  str_Replies[i] = '\n'.join(list(Replies[i]))
...
def index():
  if request.method == 'POST':
    ...
    return render_template('index.html',
                           ...
                           counter=active_user[users.index(username)])
